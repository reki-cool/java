...
import cn.bms.utils.AddBookUtils;
...
public class AddBookServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		BookForm form = AddBookUtils.requestToForm(request, BookForm.class);
		...
		Book book = new Book();
		AddBookUtils.copyBean(book, form);
		
		//给图书、作者、图书类别生成ID
		book.setBkId(AddBookUtils.generateID());
		book.setAuId(AddBookUtils.generateID());
		book.setTypeId(AddBookUtils.generateID());
		
		BusinessServiceImpl service = new BusinessServiceImpl();
		try{
			service.addBook(book);
			
			//如果Service处理成功，就跳转到网站的全局消息显示页面，为用户显示注册成功的消息
			request.setAttribute("message", "添加成功!!!将在3秒以后自动跳转到总览图书列表<meta http-equiv='refresh' content='3; url="+ request.getContextPath() +"/FindAllBooksAdminServlet'");
			...
			request.setAttribute("message", "服务器出现未知错误!!将在3秒以后自动返回图书添加界面<meta http-equiv='refresh' content='3; url="+ request.getContextPath() +"/AddBookAdminServlet'");
			request.getRequestDispatcher("/message.jsp").forward(request, response);
			return;
		}
